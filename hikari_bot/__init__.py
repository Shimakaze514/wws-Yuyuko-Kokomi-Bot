...
import httpx
from kokomi2.plugins.nonebot_plugin_kokomi import __plugin_version__
...
async def send_random_ocr_image(bot: Bot, ev: MessageEvent):
    try:
        img = await get_Random_Ocr_Pic_Local()
...
async def remind(bot: Bot):
    ...
    for each in superid:
        await bot.send_private_msg(user_id=int(each), message=f'澪喵BOT已上线，当前HikariBOT版本{__bot_version__},Hikari内核版本{__version__},kokomi版本{__plugin_version__}')
