...
def main(
    ...
    if result['data']['pr']['battle_type']['pvp'] == {}:
        ...
        index = 'pvp'
        text_list.append([(602+14-150, 405+38), 'PVP', (0, 0, 0), 1, 55])
    ...
    res_img = cv2.imread(os.path.join(
        file_path, 'png', 'background', 'wws_ship.png'), cv2.IMREAD_UNCHANGED)
    ...
    y0 = 2719+12
    temp_data = result['data']['pr']['battle_type'][index]
    w = x_coord(str(temp_data['uwin']), fontStyle)
    text_list.append(
        [(680-w/2, y0 + 89*0), str(temp_data['uwin']), (0, 0, 0), 1, 55])
    w = x_coord(str(temp_data['udmg']), fontStyle)
    text_list.append(
        [(680-w/2, y0 + 89*1), str(temp_data['udmg']), (0, 0, 0), 1, 55])
    w = x_coord(str(temp_data['ufrag']), fontStyle)
    text_list.append(
        [(680-w/2, y0 + 89*2), str(temp_data['ufrag']), (0, 0, 0), 1, 55])
    w = x_coord(str(temp_data['swin']), fontStyle)
    text_list.append(
        [(1107-w/2, y0 + 89*0), str(temp_data['swin']), (0, 0, 0), 1, 55])
    w = x_coord(str(temp_data['sdmg']), fontStyle)
    text_list.append(
        [(1107-w/2, y0 + 89*1), str(temp_data['sdmg']), (0, 0, 0), 1, 55])
    w = x_coord(str(temp_data['sfrag']), fontStyle)
    text_list.append(
        [(1107-w/2, y0 + 89*2), str(temp_data['sfrag']), (0, 0, 0), 1, 55])
    if temp_data['uwin'] >= temp_data['swin']:
        w = x_coord(
            '+' + str(round(temp_data['uwin']-temp_data['swin'], 2)), fontStyle)
        text_list.append([(1540-w/2, y0 + 89*0), '+' +
                         str(round(temp_data['uwin']-temp_data['swin'], 2)), (49, 128, 0), 1, 55])
    else:
        w = x_coord(
            str(round(temp_data['uwin']-temp_data['swin'], 2)), fontStyle)
        text_list.append(
            [(1540-w/2, y0 + 89*0), str(round(temp_data['uwin']-temp_data['swin'], 2)), (205, 51, 51), 1, 55])
    if temp_data['udmg'] >= temp_data['sdmg']:
        w = x_coord(
            '+' + str(round(temp_data['udmg']-temp_data['sdmg'], 2)), fontStyle)
        text_list.append([(1540-w/2, y0 + 89*1), '+' +
                         str(round(temp_data['udmg']-temp_data['sdmg'], 2)), (49, 128, 0), 1, 55])
    else:
        w = x_coord(
            str(round(temp_data['udmg']-temp_data['sdmg'], 2)), fontStyle)
        text_list.append(
            [(1540-w/2, y0 + 89*1), str(round(temp_data['udmg']-temp_data['sdmg'], 2)), (205, 51, 51), 1, 55])
    if temp_data['ufrag'] >= temp_data['sfrag']:
        w = x_coord(
            '+' + str(round(temp_data['ufrag']-temp_data['sfrag'], 2)), fontStyle)
        text_list.append([(1540-w/2, y0 + 89*2), '+' +
                         str(round(temp_data['ufrag']-temp_data['sfrag'], 2)), (49, 128, 0), 1, 55])
    else:
        w = x_coord(
            str(round(temp_data['ufrag'] - temp_data['sfrag'], 2)), fontStyle)
        text_list.append([(1540-w/2, y0 + 89*2), str(round(temp_data['ufrag'] -
                         temp_data['sfrag'], 2)), (205, 51, 51), 1, 55])
    if use_pr:
        w = x_coord(str(temp_data['nwin']), fontStyle)
        text_list.append(
            [(1928-w/2, y0 + 89*0), str(temp_data['nwin']), (0, 0, 0), 1, 55])
        w = x_coord(str(temp_data['ndmg']), fontStyle)
        text_list.append(
            [(1928-w/2, y0 + 89*1), str(temp_data['ndmg']), (0, 0, 0), 1, 55])
        w = x_coord(str(temp_data['nfrag']), fontStyle)
        text_list.append(
            [(1928-w/2, y0 + 89*2), str(temp_data['nfrag']), (0, 0, 0), 1, 55])
        if temp_data['value_battles_count'] == 0:
            w = x_coord(str(-1), fontStyle)
            text_list.append(
                [(1928-w/2, y0 + 89*3), str(-1), (0, 0, 0), 1, 55])
        else:
            w = x_coord(
                str(int(temp_data['personal_rating']/temp_data['value_battles_count'])), fontStyle)
            text_list.append([(1928-w/2, y0 + 89*3), str(int(temp_data['personal_rating'] /
                                                             temp_data['value_battles_count'])+1), (0, 0, 0), 1, 55])
    else:
        w = x_coord('-', fontStyle)
        text_list.append([(1928-w/2, y0 + 89*0), '-', (0, 0, 0), 1, 55])
        text_list.append([(1928-w/2, y0 + 89*1), '-', (0, 0, 0), 1, 55])
        text_list.append([(1928-w/2, y0 + 89*2), '-', (0, 0, 0), 1, 55])
        text_list.append([(1928-w/2, y0 + 89*3), '-', (0, 0, 0), 1, 55])

    fontStyle = font_list[1][80]
    w = x_coord(BOT_VERSON, fontStyle)
    text_list.append(
        [(1214-w/2, 3453), BOT_VERSON, (174, 174, 174), 1, 80])
    if os.path.exists(os.path.join(file_path, 'png', 'al_png', f'{ship_id}.png')):
        x1 = 99
        y1 = 3096
        ship_png = cv2.imread(os.path.join(
            file_path, 'png', 'al_png', f'{ship_id}.png'), cv2.IMREAD_UNCHANGED)
        ...
        y1 = 3096
        ship_png = cv2.imread(os.path.join(
            file_path, 'png', 'al_png', 'default.png'), cv2.IMREAD_UNCHANGED)
        x2 = x1 + ship_png.shape[1]
        y2 = y1 + ship_png.shape[0]
        res_img = merge_img(res_img, ship_png, y1, y2, x1, x2)
    del ship_png
    ...
    res_img = res_img.resize((1214, 1773))
    return res_img
...
async def get_png(
    ...
    try:
        # [aid,server,ship_id,ship_name,use_pr,use_ac,ac]
        async with httpx.AsyncClient() as client:
            if parameter[5]:
                url = API_URL + '/user/ship/' + \
                    f'?token={API_TOKEN}&aid={parameter[0]}&server={parameter[1]}&ship_id={parameter[2]}&use_ac=True&ac={parameter[6]}'
            else:
                url = API_URL + '/user/ship/' + \
        ...
        if result['data']['pr']['battle_type']['pvp'] == {} and result['data']['pr']['battle_type']['rank_solo'] == {}:
            return {'status': 'info', 'message': '您在该船只上没有战斗数据'}
        res_img = main(
            result=result,
            aid=parameter[0],
            server=parameter[1],
