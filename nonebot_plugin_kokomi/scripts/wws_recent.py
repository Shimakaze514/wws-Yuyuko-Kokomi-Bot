...
def main(
    ...
    res_img = cv2.imread(os.path.join(
        file_path, 'png', 'background', 'wws_recent.png'), cv2.IMREAD_UNCHANGED)
    ...
    now_time = time.strftime(
        "%Y-%m-%d %H:%M:%S", time.localtime(result['data']['time']['now_time']))
    last_update_time = time.strftime(
        "%Y-%m-%d %H:%M:%S", time.localtime(result['data']['time']['last_update_time']))
    time_str = f'数据统计区间: [{last_update_time}]--[{now_time}]'
    fontStyle = font_list[1][25]
    w = x_coord(time_str, fontStyle)
    text_list.append(
        [(1214-w/2, 963), time_str, (75, 75, 75), 1, 25])
    # achievement
    achievement_list = result['data']['achievements']
    if achievement_list != {}:
        ...
        for ach_list in achievement_list:
            ...
            if i == 10:
                text_list.append(
                    [(160+194*i, 1710), '······'.format(num), (0, 0, 0), 1, 65])
                break
            ach_png_path = os.path.join(
                file_path, 'png', 'achievement', '{}.png'.format(index))
            ach_png = cv2.imread(ach_png_path, cv2.IMREAD_UNCHANGED)
            ach_png = cv2.resize(ach_png, None, fx=1.7, fy=1.7)
            x1 = 156 + 194*i
            y1 = 1670
            x2 = x1 + ach_png.shape[1]
            y2 = y1 + ach_png.shape[0]
            res_img = merge_img(res_img, ach_png, y1, y2, x1, x2)
            text_list.append(
                [(277+194*i, 1768), 'x{}'.format(num), (0, 0, 0), 1, 35])
            i += 1
    # 船只数据
    ships_pr_list = {}
    ...
    for ship_id, ship_pr in sorts_pr_list:
        ...
        if SHIP_PREVIEW:
            ...
            text_list.append(
                [(210-w/2, y0+89*i), ship_tier, (0, 0, 0), 1, 50])
            w = x_coord(ship_name, fontStyle)
            ship_name = formate_str(ship_name, w, 290)
            text_list.append([(250, y0+89*i), ship_name, (0, 0, 0), 1, 50])
...
async def get_png(
    ...
    aid = parameter[0]
    server = parameter[1]
    battle_type = parameter[4]
    use_pr = parameter[5]

    try:
        # [aid,server,date,today,battles_type,use_pr,use_ac,ac]
        async with httpx.AsyncClient() as client:
            if parameter[6]:
                url = API_URL + '/database/recent/' + \
                    f'?token={API_TOKEN}&aid={parameter[0]}&server={parameter[1]}&date={parameter[2]}&today={parameter[3]}&use_ac=True&ac={parameter[7]}'
            else:
                url = API_URL + '/database/recent/' + \
                    f'?token={API_TOKEN}&aid={parameter[0]}&server={parameter[1]}&date={parameter[2]}&today={parameter[3]}'
        ...
        if battle_type == 'pvp':
            ...
            if result['data']['pr']['battle_type']['rank_solo'] == {}:
                return {'status': 'info', 'message': '该日期范围内没有排位战斗的数据'}
        res_img = main(
            result,
            aid,
            server,
